...
static inline void put_u16_le(uint8_t *buf, int i, uint16_t v) { // buf - pointer to can data array (msg.buf[i]) // i - starting index // v - the sensor numerical
...
void send_depth_frame(float depth_m, uint16_t &counter_ref) {
  // in case depth negative, have accounted for that
